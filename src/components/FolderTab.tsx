...
const FolderTab = ({ folder, profilesInFolder, isActive, onToggleCollapse, onSettings }: FolderTabProps) => {
    const Icon = getIcon(folder.icon);

    return (
        <button
            className={`folder-tab__button ${isActive ? 'folder-tab--active' : ''} ${folder.isCollapsed ? 'folder-tab--collapsed' : ''}`}
            style={{ color: folder.color }}
            onDoubleClick={() => onToggleCollapse(folder.id)}
        >
            <Icon />
            {folder.name}
            ...
            <div className="tab__actions">
             ...
             <svg className="folder-tab__collapse-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
