...
const GoalDefinition = () => {
    ...
    // Filter options for secondary select to exclude any primary selections
    const availableSecondaryOptions = BLUE_SPARK_OPTIONS.filter(opt => !goal.primaryBlue.includes(opt));
    // Filter options for primary select to exclude any secondary selections
    const availablePrimaryOptions = BLUE_SPARK_OPTIONS.filter(opt => !goal.secondaryBlue.includes(opt));
            ...
            <div className="space-y-4">
                <div>
                    <h3 className="form__section-title mb-2">{t('primaryBlue')}</h3>
                     <MultiSelect
                        options={translatedOptions(availablePrimaryOptions)}
                        selectedValues={goal.primaryBlue}
                        onChange={(selected) => handleGoalChange({ primaryBlue: selected })}
                    />
                </div>
                 <div className="border-t pt-4">
                    <h3 className="form__section-title mb-2">{t('secondaryStat')}</h3>
                     <MultiSelect
                        options={translatedOptions(availableSecondaryOptions)}
                        selectedValues={goal.secondaryBlue}
                        onChange={(selected) => handleGoalChange({ secondaryBlue: selected })}
                    />
                </div>
                <div className="border-t pt-4">
