...
import { faChevronLeft, faChevronRight, faClipboardQuestion, faCalculator, faSpinner } from '@fortawesome/free-solid-svg-icons';
...
const TopBreedingPair = () => {
    ...
    const { 
        setActiveBreedingPair, getActiveProfile, topBreedingPairs, sortField, setSortField,
        isCalculating
    } = useAppContext();
    ...
    const recommendedPairs = topBreedingPairs[recType];

    useEffect(() => {
        setCurrentIndex(0);
    }, [recType, sortField, topBreedingPairs]);
                ...
                <div className="top-pair__controls">
                    <div className="top-pair__toggle-group">
                     ...
                     <div className="top-pair__toggle-group">
                        ...
                        <button className={`top-pair__toggle-btn ${sortField === 'score' ? 'top-pair__toggle-btn--active' : ''}`} onClick={() => setSortField('score')}>{t('topPair.avgFinalScore')}</button>
                        <button className={`top-pair__toggle-btn ${sortField === 'individualScore' ? 'top-pair__toggle-btn--active' : ''}`} onClick={() => setSortField('individualScore')}>{t('topPair.avgIndivScore')}</button>
                    </div>
                </div>
                
                <div className={`top-pair__carousel ${isCalculating ? 'is-loading' : ''}`}>
                        ...
                        {currentPair ? (
                            <>
                                ...
                                <div className="top-pair__meta">
                                    ...
                                    <span>{t('topPair.avgFinalScore')}: {Math.round(currentPair.avgFinalScore)}</span>
                                    <span className="mx-2">|</span>
